...
async function main() {
    ...
    const hrPermissions = createdPermissions.filter((p) => ["user", "department", "team", "designation", "office", "employee_type", "employment_status"].includes(p.resource));
    ...
    const passwordHash = await bcrypt_1.default.hash(superAdminPassword, 12);
